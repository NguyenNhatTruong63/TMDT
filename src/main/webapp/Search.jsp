            ...
            <div class="row">
                ...
                <div class="rows cat-products clearfix">
                    ...
                    <% List<Product> list = (List<Product>) request.getAttribute("list");
                        ...
                        if (list == null) { %>
                    <h3>Không tìm thấy sản phẩm có từ khóa là "<%=text%>" trong <%=cate%>
                    </h3>
                    <% } else { %>
                    <h3>Sản phẩm có từ khóa là "<%=text%>" trong <%=cate%>
                    </h3> <%
                    for (Product p : list) { %>
                        ...
                        <div class="product-thumb">
                            <div class="image">
                                <a href="Product?id=<%=p.getId()%>">
                                    ...
                                    <%} else { %>
                                    <img class="img-responsive hover-img" src="<%= p.getImages().get(1)%>" title="sach" alt="sach">
                                    <% } %>
                                </a>
                                <div class="product-item-addcart">
                                    <a href="Cart?id=<%=p.getId()%>" class="addcart"><span>Thêm vào giỏ hàng</span></a>
                                </div>
                            </div>
                            <div class="thumb-description clearfix">
                                <div class="caption">
                                    <h4 class="product_name"><a href="Product?id=<%=p.getId()%>"><%= p.getName()%></a></h4>
                                    <p class="price"><%= currencyFormatter.format(p.getPrice())%>&nbsp;VNĐ</p>
                                </div>
                            </div>
                        </div>
                    </div>
