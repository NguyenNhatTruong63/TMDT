                ...
                <div class="row">
                    <div class="col-sm-6 product-left">
                        <div class="thumbnails">
                            <div class="pro-image" width="360" height="540">
                                <a href="<%=product.getImages().get(0)%>" title="sach" class="thumbnail">
                                    <img src="<%=product.getImages().get(0)%>" title="sach" id="zoom" alt="sach" data-zoom-image="<%=product.getImages().get(0)%>"  />
                                </a>
                            </div>
                            <div id="additional-carousel" class="owl-carousel owl-theme thumbnails-gellery clearfix">
                                <% for(String link: product.getImages()) { %>
                                    ...
                                    <a href="<%=link%>" title="party wear" class="elevatezoom-gallery" data-image="<%=link%>"
                                       data-zoom-image="<%=link%>"><img src="<%=link%>" title="party wear" alt="party wear" width="90" height="100" />
                                    </a>
                                </div>
                                <% } %>
                            </div>
                    ...
                    <div class="col-sm-6 right-info">
                        ...
                        <ul class="list-unstyled product_price">
                            ...
                            <li>
                                <span class="pro_oldprice" style="text-decoration: line-through;"><%=currencyFormatter.format(product.getPrice())%>&nbsp;VNĐ</span>
                                <span class="pro_price"><%=currencyFormatter.format(product.getPrice_buy())%>&nbsp;VNĐ</span>
                            </li>
                            <% } else { %>
                            <span class="pro_price"><%=currencyFormatter.format(product.getPrice_buy())%>&nbsp;VNĐ</span>
                            <% } %>
                                ...
                                <div class="product-thumb transition">
                                    <div class="image">
                                        <a href="Product?id=<%=p.getId()%>">
                                            <img src="<%=p.getImages().get(0)%>" alt="sach" title="sach" class="img-responsive" />
                                            <% if(p.getImages().size()==1){ %>
                                                <img class="img-responsive hover-img" src="<%=p.getImages().get(0)%>" title="sach" alt="sach" />
                                            <% } else { %>
                                                <img class="img-responsive hover-img" src="<%=p.getImages().get(0)%>" title="sach" alt="sach" />
                                            <% } %>
                                        </a>
                                        <div class="product-item-addcart">
                                            <button type="button" class="addcart" onclick="cart.add();"><span>Thêm vào giỏ hàng</span></button>
                                        </div>
                                    </div>
                                    <div class="thumb-description clearfix">
                                        <div class="caption">
                                            ...
                                            <span class="pro_oldprice" style="text-decoration: line-through;"><%=currencyFormatter.format(p.getPrice())%>>&nbsp;VNĐ</span>
                                            <p class="price"><%=currencyFormatter.format(p.getPrice_buy())%>&nbsp;VNĐ</p>
                                            <% } else { %>
                                            <p class="price"><%=currencyFormatter.format(p.getPrice_buy())%>&nbsp;VNĐ</p>
                                            <% } %>
                                        </div>
    ...
    <script type="text/javascript">
        ...
        $('#additional-carousel').each(function () {
            ...
            const sliderOptions = {
                loop: false,
                nav: true,
                navText: ['<i style="margin-top: 10px" class="fa fa-angle-left fa-5x"></i>', '<i style="text-align: center" class="fa fa-angle-right fa-5x"></i>'],
                dots: false,
                items: 4,
